BEGIN;
-- User feedback: setelah Scan QR / Scan Wajah / Input PIN, jangan minta ketik PIN LAGI --
-- masing-masing dari 3 metode itu HARUS langsung checkin/checkout dalam satu langkah,
-- persis pola HSSE Fusion4 (identifikasi = otorisasi, satu aksi).
--
-- Jadi checkin_by_qrcode/checkout_by_qrcode: QR fisik / wajah yang match dianggap cukup
-- (sama seperti attendance-fusion4.html punya Fusion4 -- sudah standar yang diterima buat
-- kelompok app ini). checkin_by_pin/checkout_by_pin: PIN yang diketik ITU SENDIRI yang jadi
-- identifikasi (bukan cari-nama dulu baru ketik PIN lagi) -- makanya wajib IsActive=true dan
-- wajib unik di antara karyawan aktif (dicek eksplisit, walau saat ini belum ada tabrakan).
--
-- Aksi checkin/checkout/find_by_qrcode versi lama (2 langkah) TETAP ada, gak dihapus --
-- gak ada yang gantung ke situ tapi ngilangin kode kerja tanpa alasan cuma nambah resiko.
