BEGIN;
-- Feedback: "Material digunakan" nyampur Consumable (Masker) sama Tools (Circular Saw, Sekop)
-- jadi satu daftar -- padahal rulesnya beda: Material/Consumables cukup input qty (udah ada),
-- Tools/Heavy Equipment butuh qty JUGA plus check-in/check-out yang ngitung jam pakai (kayak
-- Manpower). purchaseOrderDetail."ItemGroup" nyimpen kategori top-level ('Material',
-- 'Consumables','Tools','Coating',dst -- bukan "Group" spesifik dari tabel master
-- material/tools/heavyEquipment, itu field beda). Kita treat 'Tools' dan 'Heavy Equipment' sbg
-- EQUIPMENT, selain itu MATERIAL (default aman kalau ada kategori legacy/custom kayak 'Coating').

CREATE TABLE operational.equipment_checkin (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
 wo_id uuid NOT NULL REFERENCES operational.work_orders(id),
 confirmation_id bigint NOT NULL REFERENCES public."endUserReceiving"("ConfirmationID"),
...
CREATE INDEX ON operational.equipment_checkin(wo_id);
CREATE UNIQUE INDEX equipment_checkin_one_open_per_item ON operational.equipment_checkin(confirmation_id) WHERE check_out_at IS NULL;
...
BEGIN
 ...
 ELSIF p_action='list_material_balance' THEN
  -- Sisa material/alat per item yang masih bisa dipakai/dikembalikan buat WO ini (gak di-scope
  -- ke tanggal). 'category' EQUIPMENT (Tools/Heavy Equipment) vs MATERIAL (sisanya, termasuk
  -- kategori legacy/custom kayak 'Coating') dipake client buat misahin section + nampilin
  -- checkin jam pakai cuma buat EQUIPMENT.
  woid:=(p_data->>'woId')::uuid;
  RETURN coalesce((SELECT jsonb_agg(jsonb_build_object(
 ...
 ELSIF p_action='save_material_usage' THEN
  ...
  IF NOT EXISTS(SELECT 1 FROM public."endUserReceiving" WHERE "ConfirmationID"=cid AND "woID"=woid) THEN
   RAISE EXCEPTION 'Item material tidak ditemukan untuk WO ini';
  END IF;
