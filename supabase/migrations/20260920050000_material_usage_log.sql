BEGIN;
-- Feedback: material yang diterima end user (endUserReceiving) sekarang bisa DIKEMBALIKAN ke
-- gudang (materialReturn) atau DIGUNAKAN di lapangan. Yang belum ada: cara input pemakaian
-- harian material/consumables dari Daily Progress (progress.html), dengan info sisa
-- (balance = diterima - dikembalikan - dipakai), biar site engineer bisa catat langsung tanpa
-- balik ke end-user-receiving.html.
CREATE TABLE operational.material_usage_log (
...
CREATE INDEX ON operational.material_usage_log(wo_id,usage_date);
CREATE INDEX ON operational.material_usage_log(confirmation_id);
...
CREATE OR REPLACE FUNCTION public.op_daily_report(p_token text, p_action text, p_data jsonb DEFAULT '{}'::jsonb)
 ...
 SET search_path TO ''
AS $function$
DECLARE actor bigint; n integer:=0; aid uuid; did uuid; sid uuid; pid uuid; phid uuid; wid uuid; woid uuid; pjid uuid; cid bigint; uid uuid; balance numeric; detail operational.sms_item_details; parent operational.sms_item_details; hdr record;
BEGIN
 ...
 ELSIF p_action='list_material_balance' THEN
  -- Sisa material per item yang masih bisa dipakai/dikembalikan buat WO ini (gak di-scope ke
  -- tanggal -- material bisa dipakai berhari-hari sampai habis atau dikembalikan ke gudang).
  woid:=(p_data->>'woId')::uuid;
  RETURN coalesce((SELECT jsonb_agg(jsonb_build_object(
    'confirmationId',x."ConfirmationID",'itemDescription',x."ItemDescription",'unit',x."Unit",'balance',x.balance
   ) ORDER BY x."ItemDescription")
   FROM (
    SELECT eur."ConfirmationID",pod."ItemDescription",pod."Unit",
     eur."QtyConfirmed"
