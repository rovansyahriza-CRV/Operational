BEGIN;
-- PIC/Foreman yang login ke Team Check-in (username+password) sudah terautentikasi lewat
-- login itu sendiri -- gak perlu scan QR/Wajah/PIN buat check-in DIRINYA SENDIRI. Begitu dia
-- pilih WO, otomatis checkin_self dipanggil dari frontend (silent, best-effort). Gak pernah
-- raise exception ke frontend biar gak ganggu alur pilih WO -- semua kondisi (sudah checkin di
-- WO ini, lagi checkin di WO lain, akun gak terhubung ke karyawanTbl aktif) dikembalikan
-- sebagai status di response, bukan error.
