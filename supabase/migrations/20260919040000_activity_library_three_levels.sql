BEGIN;
-- activity_library dinaikkan dari 2 level (kategori+nama) jadi tree 3 level:
-- Kategori (Mechanical/Civil/...) -> Aktivitas (Welding/Blasting/...) -> Detail Sub (Fitup/Primer/...).
-- Detail Sub sifatnya opsional, cuma dipakai aktivitas yang butuh breakdown lebih halus.
-- Baris lama yang sudah dipakai (Mechanical/Tubular Cleaning, sudah direferensikan sms_item_details)
-- dipertahankan ID-nya, cuma dipindah jadi anak dari kategori "Mechanical" yang baru dibuat.

ALTER TABLE operational.activity_library ADD COLUMN parent_id uuid REFERENCES operational.activity_library(id);
ALTER TABLE operational.activity_library ALTER COLUMN category DROP NOT NULL;

DO $$
DECLARE r record; catid uuid;
BEGIN
 FOR r IN SELECT id, category, created_by FROM operational.activity_library WHERE parent_id IS NULL LOOP
  SELECT id INTO catid FROM operational.activity_library WHERE parent_id IS NULL AND name=r.category AND id<>r.id;
  IF catid IS NULL THEN
   INSERT INTO operational.activity_library(name,created_by) VALUES(r.category,r.created_by) RETURNING id INTO catid;
  END IF;
  UPDATE operational.activity_library SET parent_id=catid WHERE id=r.id;
 END LOOP;
END $$;

ALTER TABLE operational.activity_library DROP CONSTRAINT activity_library_category_name_key;
ALTER TABLE operational.activity_library DROP COLUMN category;
ALTER TABLE operational.activity_library ADD CONSTRAINT activity_library_parent_name_key UNIQUE(parent_id,name);
CREATE UNIQUE INDEX activity_library_top_level_name_key ON operational.activity_library(name) WHERE parent_id IS NULL;

-- Seed master data (kategori -> aktivitas -> detail sub) sesuai daftar yang diberikan.
DO $$
DECLARE seeder bigint:=21;
 cat_id uuid; act_id uuid;
 cat text; act text; details text[];
BEGIN
 FOR cat, act, details IN VALUES
  ('Mechanical','Welding',ARRAY['Fitup','Welding']),
  ('Mechanical','Blasting',ARRAY['Surface Preparation','Blasting']),
  ('Mechanical','Painting',ARRAY['Primer','2nd Coat','3rdCoat','Top Coat']),
  ('Mechanical','Tubular Cleaning',ARRAY['Internal Cleaning','External Cleaning']),
  ('Civil','Excavation',NULL),
  ('Civil','Back Filling',NULL),
  ('Civil','Rebar',NULL),
  ('Civil','Bekisting',NULL),
  ('Civil','Pouring Concrete',NULL),
  ('Support','Lifting',NULL),
  ('Support','Scaffolding',NULL),
  ('Non Destructive Test','Radiography',NULL),
  ('Non Destructive Test','MT PT',NULL),
  ('Non Destructive Test','UT',NULL),
  ('Testing','Hydrotest',NULL),
  ('Testing','Pneumatic Test',NULL),
  ('Testing','Holiday test',NULL)
 LOOP
  INSERT INTO operational.activity_library(name,created_by) VALUES(cat,seeder)
   ON CONFLICT (name) WHERE parent_id IS NULL DO NOTHING;
  SELECT id INTO cat_id FROM operational.activity_library WHERE parent_id IS NULL AND name=cat;

  INSERT INTO operational.activity_library(parent_id,name,created_by) VALUES(cat_id,act,seeder)
   ON CONFLICT ON CONSTRAINT activity_library_parent_name_key DO NOTHING;
  SELECT id INTO act_id FROM operational.activity_library WHERE parent_id=cat_id AND name=act;

  IF details IS NOT NULL THEN
   INSERT INTO operational.activity_library(parent_id,name,created_by)
    SELECT act_id,d,seeder FROM unnest(details) d
    ON CONFLICT ON CONSTRAINT activity_library_parent_name_key DO NOTHING;
  END IF;
 END LOOP;
END $$;
