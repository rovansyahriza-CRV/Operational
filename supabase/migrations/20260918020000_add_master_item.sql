BEGIN;
-- Let PIC "Operational Master Komersial" append a single item to an already-committed
-- contract (e.g. filling in a Provision Sum group once the real vendor invoice is known),
-- without re-running the whole bulk Excel import or mutating existing items/revisions.
...
BEGIN
 ...
 ELSIF p_action IN ('import_master','add_master_item') THEN actor:=operational.check_session(p_token,ARRAY['operational master komersial']);
