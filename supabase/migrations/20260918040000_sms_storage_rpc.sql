BEGIN;
-- SMS (Site Measurement Sheet) tahap 2: simpan beneran ke Supabase, gak lagi cuma di
-- localStorage/draft JSON. Nomor SMS sekarang server-authoritative (sequence_no per WO,
-- auto-increment), jadi gak ada lagi race condition dua orang generate nomor bareng.
-- Bukti approval (PDF) disimpan sebagai base64 langsung di kolom (bukan Supabase Storage)
-- supaya tetap konsisten sama pola app ini: semua tulis-menulis lewat RPC yang divalidasi
-- token/PIC, bukan lewat jalur otorisasi terpisah (Storage RLS butuh JWT yang app ini
-- gak punya karena pakai sesi kustom, bukan Supabase Auth).

ALTER TABLE operational.sms_documents ADD COLUMN evidence_data text;
...
smsid uuid; smsrow operational.sms_documents; sms_seq integer; sms_rev text; routing_ok boolean; target_status text; evidence_b64 text;
BEGIN
 ...
 ELSIF p_action='save_sms' THEN
  ...
  SELECT * INTO w FROM operational.work_orders WHERE id=wid;
  IF w.id IS NULL THEN RAISE EXCEPTION 'WO tidak ditemukan'; END IF;
  IF NOT EXISTS(SELECT 1 FROM operational.wo_items WHERE wo_id=wid) THEN RAISE EXCEPTION 'WO belum punya item, gak bisa bikin SMS'; END IF;
  ...
  DELETE FROM operational.sms_items WHERE sms_id=smsid;
  ...
  SELECT smsid,wid,wi.package_id,wi.commercial_item_id,wi.code_snapshot,wi.description_snapshot,wi.unit_snapshot,wi.unit_price_snapshot,wi.qty
  FROM operational.wo_items wi WHERE wi.wo_id=wid;
