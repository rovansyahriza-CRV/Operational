BEGIN;
-- Bug: list_material_usage (yang ngisi tabel "Material & Consumables Digunakan" di Daily
-- Report/PDF) belum difilter kategori -- entri lama punya Tools (Circular Saw, Sekop) yang
-- dicatat lewat qty sebelum fitur split kemarin (migrasi 20260921010000) masih ikut nongol
-- nyampur di tabel Material. Data lamanya dibiarin di material_usage_log (bukan data ngaco,
-- history transaksi asli), tapi query-nya sekarang exclude kategori EQUIPMENT (Tools/Heavy
-- Equipment) biar konsisten sama rule baru: Tools/Heavy Equipment cuma dilaporkan lewat
-- equipment_checkin (checkin/checkout jam pakai), bukan lewat material_usage_log.
