BEGIN;
-- Feedback: Detail Laporan di Daily Progress ngebiarin Pemilik Proyek/Kontraktor Pelaksana/
-- Konsultan Pengawas/No.SPK diketik ulang tiap WO -- padahal itu data proyek yang mestinya cuma
-- diisi sekali. Sekarang cuma Lokasi Proyek + Mulai/Akhir WO yang manual di Daily Progress;
-- 4 field itu dipindah ke halaman baru "Project List" (project-list.html), dikelola sekali per
-- proyek lewat op_api (list_projects/save_project_info). No. SPK/Kontrak sekarang diambil
-- langsung dari operational.contracts.number (udah ada sejak kontrak diimpor, dulu malah
-- diketik ulang manual & duplikat di operational.projects.contract_number).
...
BEGIN
 ...
 ELSIF p_action='save_project_info' THEN
  ...
  IF NOT EXISTS(SELECT 1 FROM operational.projects WHERE id=pid) THEN RAISE EXCEPTION 'Proyek tidak ditemukan'; END IF;
...
-- op_daily_report: get_report_header ambil No.SPK dari contracts.number (bukan
-- projects.contract_number yang lama, sekarang gak dipakai lagi); save_report_header cuma
-- masih nerima location + startDate/endDate (client/contractorName/supervisorConsultant/
-- contractNumber dipindah ke op_api save_project_info / sumbernya contracts.number).
