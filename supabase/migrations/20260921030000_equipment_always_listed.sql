BEGIN;
-- Dua bug dari trial user:
-- 1) list_material_received belum nyertain 'category' -- section "Material diterima end user"
--    nampilin apa aja termasuk Tools, bikin label salah kalau yang diterima cuma alat.
--    Client sekarang bisa misahin jadi "Material & Consumables Diterima" vs "Alat & Tools
--    Diterima" pakai field category ini.
-- 2) list_material_balance filter WHERE balance>0 bikin alat (Tools/Heavy Equipment) HILANG dari
--    daftar begitu qty-nya "habis kepake" di material_usage_log (dari testing lama sebelum ada
--    split kemarin). Alat itu konsepnya dipinjam/checkin-checkout, bukan dikonsumsi -- jadi
--    harus SELALU muncul di daftar selama masih ada di endUserReceiving buat WO ini, gak peduli
--    hasil hitungan balance (yang buat alat emang gak relevan lagi).
