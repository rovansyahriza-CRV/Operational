BEGIN;
-- Modul Daily Progress terpisah: butuh liat SEMUA leaf breakdown lintas semua item
-- dalam satu SMS sekaligus (bukan per item kayak read_sms_item_details), biar Project
-- Control bisa update progress banyak sub-item dalam satu layar tanpa buka dialog
-- Detail satu-satu. Path (Group > Joint > Leaf) dan kode/uraian item induk disusun
-- di frontend dari data mentah yang dikembalikan sini.
