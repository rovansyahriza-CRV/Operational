BEGIN;
-- Dari review format "Laporan Harian Konstruksi" standar: (1) header proyek (Owner, Lokasi,
-- Kontraktor Pelaksana, Konsultan Pengawas, No. SPK) belum pernah kekirim ke Supabase --
-- form Project List di desktop app cuma nyimpen ke localStorage browser, jadi field-field
-- itu selalu kosong di tabel operational.projects. Daripada rombak alur Project List yang
-- udah jalan aktif, form kecil terpisah ditambahkan di progress.html buat isi/update field
-- ini sekali per WO. (2) Cuaca dipecah jadi 3 shift (Pagi/Siang/Lembur) masing-masing dengan
-- kondisi cuaca, suhu, dan jam kerja efektif -- bukan 1 nilai umum per hari lagi.

ALTER TABLE operational.projects
 ADD COLUMN supervisor_consultant text NOT NULL DEFAULT '',
 ADD COLUMN contractor_name text NOT NULL DEFAULT '';

-- sms_item_weather: pecah jadi per shift. Data lama (1 baris CERAH milik user, sebelumnya
-- 1 baris = 1 hari) dianggap shift PAGI secara default lewat kolom baru ini.
ALTER TABLE operational.sms_item_weather
 ADD COLUMN shift text NOT NULL DEFAULT 'PAGI' CHECK(shift IN ('PAGI','SIANG','LEMBUR')),
 ADD COLUMN temperature_c numeric,
 ADD COLUMN effective_hours numeric CHECK(effective_hours IS NULL OR effective_hours>=0);
ALTER TABLE operational.sms_item_weather DROP CONSTRAINT sms_item_weather_sms_item_id_report_date_key;
ALTER TABLE operational.sms_item_weather ADD CONSTRAINT sms_item_weather_item_date_shift_key UNIQUE(sms_item_id,report_date,shift);
ALTER TABLE operational.sms_item_weather ALTER COLUMN shift DROP DEFAULT;
...
BEGIN
 ...
 ELSIF p_action='get_report_header' THEN
  ...
  SELECT w.number AS wo_number,w.title AS wo_title,p.id AS project_id,p.name AS project_name,p.client,p.location,
   p.contract_number,p.start_date,p.end_date,p.supervisor_consultant,p.contractor_name
  INTO hdr
  ...
  RETURN jsonb_build_object('woNumber',hdr.wo_number,'woTitle',hdr.wo_title,'projectName',hdr.project_name,
   'client',hdr.client,'location',hdr.location,'contractNumber',hdr.contract_number,'startDate',hdr.start_date,
   'endDate',hdr.end_date,'supervisorConsultant',hdr.supervisor_consultant,'contractorName',hdr.contractor_name);
