BEGIN;
-- Master: "update in place" for an already-committed contract (same revision, no new
-- contract/revision row). Groups are always replaced (never referenced by wo_items, safe).
-- Items are upserted by code (keeps id stable so existing WO snapshots/links stay valid);
-- items missing from the new data are deleted only if no WO already references them,
-- otherwise left orphaned (parent_id NULL) rather than silently destroyed.
--
-- WO: "revise" creates a NEW work_orders row (same contract+number, incremented revision,
-- fresh DRAFT) instead of mutating an existing WO in place -- the original stays untouched
-- as an immutable historical record. This mirrors how contracts/sms_documents already use
-- a revision column.

ALTER TABLE operational.work_orders ADD COLUMN revision text NOT NULL DEFAULT '01' CHECK(btrim(revision)<>'');
ALTER TABLE operational.work_orders DROP CONSTRAINT work_orders_contract_id_number_key;
ALTER TABLE operational.work_orders ADD CONSTRAINT work_orders_contract_id_number_revision_key UNIQUE(contract_id,number,revision);
...
BEGIN
 ...
 ELSIF p_action IN ('save_wo','list_wo','read_wo','revise_wo') THEN actor:=operational.check_session(p_token,ARRAY['operational wo']);
