BEGIN;
-- Feedback: "Hari ke-X dari Y hari" di Daily Report seharusnya dihitung dari durasi WO
-- (Tanjung Batu area services, dsb -- eksekusi konkret di lapangan), bukan durasi kontrak
-- blanket keseluruhan project (bisa 2-3 tahun, banyak WO di dalamnya). Owner/Lokasi/
-- Kontraktor/Konsultan/No.SPK tetap di level project (jarang beda antar WO dalam 1 project),
-- cuma start/end date yang pindah ke level WO.
ALTER TABLE operational.work_orders
 ADD COLUMN start_date date,
 ADD COLUMN end_date date,
 ADD CONSTRAINT wo_dates_valid CHECK((start_date IS NULL AND end_date IS NULL) OR (start_date IS NOT NULL AND end_date IS NOT NULL AND end_date>=start_date));
