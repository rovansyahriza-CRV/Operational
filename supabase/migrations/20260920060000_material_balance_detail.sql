BEGIN;
-- Feedback: UI "Material digunakan" cuma nampilin nama item + sisa di dropdown -- gak kebaca
-- jelas. list_material_balance sekarang ikut nyertain qtyConfirmed (diterima) dan qtyUsed
-- (total dipakai sampai sekarang) per item, biar client bisa render kartu Diterima/Terpakai/Sisa
-- kayak tabel tanpa RPC tambahan.
...
BEGIN
 ...
 ELSIF p_action='list_material_balance' THEN
  ...
  -- Nyertain qtyConfirmed+qtyUsed juga biar UI bisa nampilin Diterima/Terpakai/Sisa (bukan cuma
  -- sisa) tanpa RPC tambahan.
  woid:=(p_data->>'woId')::uuid;
  RETURN coalesce((SELECT jsonb_agg(jsonb_build_object(
    'confirmationId',x."ConfirmationID",'itemDescription',x."ItemDescription",'unit',x."Unit",
    'qtyConfirmed',x."QtyConfirmed",'qtyUsed',x.qty_used,'balance',x.balance
