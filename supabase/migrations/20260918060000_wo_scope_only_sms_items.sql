BEGIN;
-- WO jadi dokumen scope/header doang: gak wajib item/qty/harga lagi buat disimpan.
-- SMS yang jadi tempat item + qty beneran dicatat (hasil survei bareng end user & contractor),
-- dan itulah acuan resmi buat invoicing progress pekerjaan aktual -- bukan WO.
-- sms_items sekarang diisi langsung dari draft yang dipilih user (snapshot dari commercial_items),
-- bukan lagi hasil copy otomatis dari wo_items.
...
BEGIN
 ...
 ELSIF p_action='save_sms' THEN
  ...
  SELECT coalesce(count(*),0) INTO n FROM operational.wo_packages WHERE wo_id=wid;
  FOR r IN SELECT value FROM jsonb_array_elements(p_data->'items') LOOP
   needed:=coalesce(nullif(btrim(r->>'package'),''),'Paket 1');
   SELECT id INTO packid FROM operational.wo_packages WHERE wo_id=wid AND name=needed LIMIT 1;
   IF packid IS NULL THEN n:=n+1; INSERT INTO operational.wo_packages(wo_id,code,name) VALUES(wid,'P'||n,needed) RETURNING id INTO packid; END IF;
   ...
   IF NOT FOUND THEN RAISE EXCEPTION 'Item master tidak ditemukan untuk salah satu baris SMS'; END IF;
  END LOOP;
