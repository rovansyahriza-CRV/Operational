BEGIN;
-- Feedback: gabung "Data Proyek" (Owner/Kontraktor/Konsultan) ke tab "Project List" yang udah
-- ada di index.html (bukan halaman terpisah lagi) -- sekali input lengkap. Tab itu sebelumnya
-- 100% localStorage doang (bima-spms-projects-v1), gak nyambung ke Supabase sama sekali.
-- operational.projects TERNYATA udah lengkap kolomnya dari awal (contract_number, start_date,
-- end_date, status, smms_project_id) -- cuma belum pernah diekspos lewat RPC. Sekarang
-- list_projects/save_project_info nyertain semua field itu, dan save_project_info upsert
-- by CODE (bukan projectId) biar tab Project List bisa langsung create-or-update dalam satu
-- aksi (dulu harus tau id dulu).
--
-- smms_project_id disiapkan buat nyambung ke public.request.PROJECTID (SMMS) -- penomorannya
-- udah kelihatan cocok (project code '014'/'015' vs PROJECTID 14/15) tapi belum ada FK/link
-- resmi. Field ini baru DIEKSPOS (readonly di sisi Operational buat sekarang), belum ada logic
-- otomatis nyamain -- itu manual diisi kalau admin mau nyambungin.
--
-- op_list_projects_public ditambah (public, no-token, sama pola kayak op_list_work_orders_public)
-- biar modul lain (SMMS-BIMA, Fusion4 SmartGate) BISA baca data ini nanti kalau mau -- belum ada
-- perubahan di repo mereka sekarang, ini cuma nyiapin sisi Operational.
CREATE OR REPLACE FUNCTION public.op_api(p_token text, p_action text, p_data jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
 IF p_action IN ('contracts','master','list_projects') THEN
 ...
 ELSIF p_action='save_project_info' THEN actor:=operational.check_session(p_token,ARRAY['operational master komersial','operational wo']);
 ...
 ELSIF p_action='list_projects' THEN
  RETURN coalesce((SELECT jsonb_agg(jsonb_build_object(
   ...
   ) ORDER BY p.name) FROM operational.projects p),'[]');
 ELSIF p_action='save_project_info' THEN
  IF nullif(btrim(p_data->>'code'),'') IS NULL THEN RAISE EXCEPTION 'Kode proyek wajib diisi'; END IF;
  IF nullif(btrim(p_data->>'name'),'') IS NULL THEN RAISE EXCEPTION 'Nama proyek wajib diisi'; END IF;
  INSERT INTO operational.projects(code,name,client,location,contractor_name,supervisor_consultant,contract_number,start_date,end_date,status,smms_project_id)
  VALUES(btrim(p_data->>'code'),btrim(p_data->>'name'),coalesce(p_data->>'client',''),coalesce(p_data->>'location',''),coalesce(p_data->>'contractorName',''),coalesce(p_data->>'supervisorConsultant',''),coalesce(p_data->>'contractNumber',''),nullif(p_data->>'startDate','')::date,nullif(p_data->>'endDate','')::date,coalesce(nullif(p_data->>'status',''),'ACTIVE'),nullif(p_data->>'smmsProjectId','')::bigint)
  ON CONFLICT(code) DO UPDATE SET
   name=excluded.name,client=excluded.client,location=excluded.location,contractor_name=excluded.contractor_name,
   supervisor_consultant=excluded.supervisor_consultant,contract_number=excluded.contract_number,
   start_date=excluded.start_date,end_date=excluded.end_date,status=excluded.status,smms_project_id=excluded.smms_project_id
  RETURNING id INTO pid;
  INSERT INTO operational.audit_log(employee_id,action,entity_id) VALUES(actor,'SAVE_PROJECT_INFO',pid);
...
$function$;
REVOKE ALL ON FUNCTION public.op_list_projects_public() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.op_list_projects_public() TO anon,authenticated;
COMMIT;
